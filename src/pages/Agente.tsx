...
import { getStoredMcpKey, useApiKeys } from "@/hooks/useData";
import {
  MCP_AGENTS,
  type McpAgent,
...
export default function Agente() {
  ...
  useEffect(() => {
    ...
    const stored = getStoredMcpKey(mcpKey.key_prefix);
    setApiKeyPlain(stored ?? "");
  }, [mcpKey?.id, mcpKey?.key_prefix]);

  const configDocument = useMemo(
    () => buildAgentConfigDocument(agent, apiKeyPlain, revealSecrets, mcpUrl),
    [agent, apiKeyPlain, revealSecrets, mcpUrl],
  );

  const copyDocument = useMemo(
    () => buildAgentConfigDocument(agent, apiKeyPlain, true, mcpUrl),
    [agent, apiKeyPlain, mcpUrl],
  );
      ...
      <Card className="space-y-4 sm:space-y-5">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" onClick={copyConfig} className="min-h-10 flex-1 sm:flex-none">
